...
with DAG(
    ...
    def refresh_archives_for_provider(**kwargs):
        ...
        with urlopen(provider_url) as url:
            ...
            for dataResource in data_resources:
                ...
                print(
                    "URL to download: "
                    + data_resource_content["connectionParameters"]["url"]
                )

                urllib.request.urlretrieve(
                    url_to_download, "/tmp/" + dataResource["uid"] + ".zip"
                )
                upload_file(
                    "/tmp/" + dataResource["uid"] + ".zip",
    ...
    (
        refresh_archives
        >> get_dataset_list
        >> [process_small, process_large, process_xlarge]
    )
    ...
    ] >> ala.ala_helper.get_success_notification_operator()
