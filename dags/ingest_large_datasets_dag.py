...
def get_dwca_steps(dataset_list):
    return [
        step_bash_cmd(
            "a. Download data",
            f" /tmp/download-datasets-hdfs.sh {ala_config.S3_BUCKET_DWCA} {ala_config.S3_BUCKET_AVRO} {dataset_list}",
        ),
...
def get_avro_steps(dataset_list):
    return [
        step_bash_cmd(
            ...
            f" /tmp/download-datasets-avro-hdfs.sh {ala_config.S3_BUCKET_AVRO} {dataset_list}",
        )
    ]
