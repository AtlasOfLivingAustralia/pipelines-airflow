...
def get_spark_steps(dataset_list, request_id, search_query):
    ...
    return [
        step_bash_cmd(
            "a. Download AVRO from S3",
            f" /tmp/download-avro-hdfs-for-downloads.sh {ala_config.S3_BUCKET_AVRO} {dataset_list}",
        ),
        step_bash_cmd(
            "b. Download Query JSON from S3",
            f" aws s3 cp s3://{ala_config.S3_BUCKET_AVRO}/exports/{request_id}/query.json /tmp/query.json",
        ),
        step_bash_cmd(
            "c. Run Export",
...
with DAG(
    ...
    def construct_steps_with_options(**kwargs):
        ...
        s3 = boto3.resource("s3")
        s3.Object(ala_config.S3_BUCKET_AVRO, f"exports/{request_id}/query.json").put(
            Body=json.dumps(predicate, indent=4)
        )
    ...
    (
        ...
        >> mark_as_complete
        >> read_doi
        >> [construct_email, construct_error_email]
    )
    construct_email >> completion_email >> get_success_notification_operator()
