...
class CreateDagOperator:
    ...
    def create_dag(
        ...
        with dag:
            ...
            triggered_task >> ala_helper.get_success_notification_operator()
...
for job in job_list:
    ...
    if CONFIG_KEY.TRIGGER_DAG_ID in job:
        ...
        params = {
            key: (None, None) if value == "(None, None)" else value
        ...
        instanceType = (
            job[CONFIG_KEY.PREINGEST_INSTANCE_TYPE]
            if "instanceType" in job
            else "m6g.xlarge"
        )
        ...
        extra_args = (
            job[CONFIG_KEY.PREINGEST_EXTRA_ARGS]
            if CONFIG_KEY.PREINGEST_EXTRA_ARGS in job
            else {}
        )
        load_images = (
            ala_config.LOAD_IMAGES
            if CONFIG_KEY.PREINGEST_LOAD_IMAGES not in job
            else job[CONFIG_KEY.PREINGEST_LOAD_IMAGES]
        )
        params = {
            "datasetIds": registry_entity_uid,
            CONFIG_KEY.PREINGEST_LOAD_IMAGES: load_images,
            "instanceType": instanceType,
            "extra_args": extra_args,
            "override_uuid_percentage_check": "false",
        }

    globals()[dag_id] = CreateDagOperator.create_dag(
        dag_id=dag_id,
        trigger_dag_id=trigger_dag_id,
        description=description,
        schedule=schedule,
