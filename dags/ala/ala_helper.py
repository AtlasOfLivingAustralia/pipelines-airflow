...
def get_dataset_sizing_from_datasets(bucket: str, datasets_list: list, sort_desc: bool = False):
    ...
    for dataset in datasets_list:
        ...
        for archive_file in archive_files:
            datasets_dict[dataset] = archive_file.size
            print(f"{dataset} = {archive_file.size}")
    return get_datasets_sizing(datasets=datasets_dict, sort_desc=sort_desc)
...
def step_s3_cp_file(dr, source_path, target_path, extra_args:str = ""):
    """
    ...
    Returns:
        ...
        :param extra_args: Additional arguments to pass to the AWS CLI command.
    """
    cmd_args = ["aws", "s3", "cp", source_path, target_path] + extra_args.split()

    return step_cmd_args(
        f"Copy {source_path} to {target_path} for {dr}", cmd_args
    )
