...
def step_bash_cmd(step_name, cmd, action_on_failure="TERMINATE_CLUSTER"):
    ...
    return {
        "Name": step_name,
        "ActionOnFailure": action_on_failure,
        "HadoopJarStep": {"Jar": "command-runner.jar", "Args": ["bash", "-c", cmd, "2>&1"]},
    }
...
def emr_python_step(name, full_py_cmd, action_on_failure="TERMINATE_CLUSTER"):
    ...
    return {
        ...
        "HadoopJarStep": {
            ...
            "Args": ["bash", "-c", f" sudo -u hadoop python3 {full_py_cmd} 2>&1"],
        },
    }
...
def step_s3_cp_file(dr, source_path, target_path):
    ...
    return step_cmd_args(
        f"Copy {source_path} to {target_path} for {dr}", ["aws", "s3", "cp", source_path, target_path, "2>&1"]
    )
