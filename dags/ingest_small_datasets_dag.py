...
def get_avro_steps(dataset_list):
    return [
        step_bash_cmd(
            "a. Download Verbatim AVRO",
            f" /tmp/download-datasets-avro.sh {ala_config.S3_BUCKET_AVRO} {dataset_list}",
        )
    ]
...
def get_post_image_steps(dataset_list, run_indexing=False):
    ...
    if run_indexing:
        ...
        processing.append(
            step_bash_cmd("h-2. SOLR", f" la-pipelines solr {dataset_list}")
        )
    ...
    post_processing = [
        ...
        step_bash_cmd(
            "j. Upload data",
            f" /tmp/upload-datasets.sh {ala_config.S3_BUCKET_AVRO} {dataset_list}",
        ),
        step_bash_cmd(
            "k. Upload export",
...
with DAG(
    ...
    cluster_creator = EmrCreateJobFlowOperator(
        ...
        job_flow_overrides=cluster_setup.get_small_cluster(
            DAG_ID, "bootstrap-ingest-small-actions.sh"
        ),
