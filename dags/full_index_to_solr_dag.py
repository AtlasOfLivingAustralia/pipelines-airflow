...
def get_spark_steps(
    ...
    return [
        ...
        s3_cp(
            ...
            "hdfs:///pipelines-outlier",
            action_on_failure="CONTINUE",
        ),
        s3_cp(
            "e. Copy Annotations from S3",
            f"s3://{ala_config.S3_BUCKET_AVRO}/pipelines-annotations/",
        ...
        step_bash_cmd(
            f"j. Commit the changes to the collection {solr_collection_name}",
            f'sudo -u hadoop curl -X POST "{ala_config.SOLR_URL}/{solr_collection_name}/update?commit=true&waitSearcher=true"',
        ),
        emr_python_step(
            "k. Add additional replicas",
        ...
        step_bash_cmd(
            "l. Clear S3 outlier ",
            f" sudo -u hadoop aws s3 rm s3://{ala_config.S3_BUCKET_AVRO}/pipelines-outlier/all --recursive",
        ),
        step_bash_cmd(
        ...
        s3_cp(
            "n. Copy Outliers results to S3",
            "hdfs:///pipelines-outlier",
            f"s3://{ala_config.S3_BUCKET_AVRO}/pipelines-outlier",
            action_on_failure="CONTINUE",
        ),
        s3_cp(
            "o. Copy Sampling results to S3",
            "hdfs:///pipelines-all-datasets/sampling",
            f"s3://{ala_config.S3_BUCKET_AVRO}/pipelines-all-datasets/sampling",
            action_on_failure="CONTINUE",
        ),
        s3_cp(
            "p. Copy Sampling metrics to S3",
            "hdfs:///pipelines-all-datasets/sampling-metrics.yml",
            f"s3://{ala_config.S3_BUCKET_AVRO}/pipelines-all-datasets/",
            action_on_failure="CONTINUE",
        ),
        emr_python_step(
...
with DAG(
    ...
    def get_drs_for_image_sync_index(**kwargs):
        ...
        if time_range and (time_range[0] or time_range[1]):
            ...
            avro_drs = ala_helper.list_drs_index_avro_in_bucket(
                bucket=bucket, time_range=[one_week_before_index_date, None]
            )
            logging.info(
    ...
    with TaskGroup(
        ...
        def construct_full_index_steps(**kwargs):
            ...
            if not skip_image_sync:
                ...
                if 0 < len(dataset_list) <= ala_config.MIN_DRS_PER_BATCH:
                    logging.info(
                        f"Found {len(dataset_list)} datasets for image sync, performing adding image sync steps"
                    )
                    spark_steps = ala_helper.get_image_sync_steps(
                        s3_bucket_avro=ala_config.S3_BUCKET_AVRO, dataset_list=datasets
                    )
                elif len(dataset_list) > ala_config.MIN_DRS_PER_BATCH:
                    logging.info(
                        f"Found {len(dataset_list)} datasets for image sync, skipping image sync steps in this cluster"
        ...
        (
            construct_full_index_steps
            >> cluster_creator_task
            >> step_adder_task
            >> step_adder_wait_task
